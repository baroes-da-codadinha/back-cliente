...
const cadastrarCarrinho = async (req, res) => {
    ...
    try {
        const encontrarProduto = await knex('produtos').where({ id });

        if (!encontrarProduto) { 
            return res.status(404).json('Produto não foi encontrado.');
        }

        const carrinho = await knex('carrinho').insert({ consumidor_id: consumidor.id, produto_id: id, nome_produto: encontrarProduto.nome, preco: encontrarProduto.preco, quantidade });
